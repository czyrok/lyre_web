...
impl<'a> UseCase<(), ProjectContextsDto> for GetRelevantProjectContextsUseCase<'a> {
    async fn run(&self, _: ()) -> Result<ProjectContextsDto, ServerFunctionError> {
        let project_contexts = self.project_service.get_relevant_project_contexts(3);
